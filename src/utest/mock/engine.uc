...
export const mock = {
	 ...
	 * @param {dict<any>} snap - The snapshot object returned by `mock.snapshot()`.
	 */
	restore: (snap) => mock_obj.restore(snap),
	 ...
	 * @returns {any} The return value of the callback.
	 */
	inject_builtin: (name, fn, cb) => mock_obj.inject_builtin(name, fn, cb),
	 ...
	 * @returns {any} The return value of the callback.
	 */
	inject: (name, state, cb) => mock_obj.inject(name, state, cb),

	/**
	 * Injects temporary mock state for multiple modules during a callback.
	 *
	 * @param {dict<dict<any>>} states - Map of module name to mock state configuration.
	 * @param {function(dict<any>): any} cb - Callback receiving a map of proxies.
	 * @returns {any} The return value of the callback.
	 */
	inject_all: (states, cb) => mock_obj.inject_all(states, cb),

	/**
	 * Global (persistent) patching utilities. Unlike `inject`, patches applied
	 * here survive across test boundaries until explicitly removed with `unpatch`.
	 */
	global: {
		/**
		 * Permanently patches a module with mock state.
		 ...
		 * @returns {any} The configured proxy.
		 */
		patch: (name, state) => mock_obj.global.patch(name, state),
		 ...
		 * @param {string} name - The name of the module to unpatch.
		 */
		unpatch: (name) => mock_obj.global.unpatch(name),
		 ...
		 * @param {any} fn - The mock value.
		 */
		patch_builtin: (name, fn) => mock_obj.global.patch_builtin(name, fn),
		 ...
		 * @param {string} name - The name of the built-in to unpatch.
		 */
		unpatch_builtin: (name) => mock_obj.global.unpatch_builtin(name),
	},
};
