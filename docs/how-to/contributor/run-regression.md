...
`examples/<prefix>_config.uc` alongside each `<prefix>_test.uc`. `make test`
...
```bash
make test ARGS="examples/unit/15_mymod_test.uc"
```
...
- Run `make meta-test` after every non-trivial change to catch regressions
  early.
